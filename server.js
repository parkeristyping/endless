...
<div class="card">
  ...
  <table>
    ...
    ${rows.map(r => `<tr><td>${r.date}</td><td>${r.model}</td><td>${r.requests.toLocaleString()}</td><td>${r.input.toLocaleString()}</td><td>${r.output.toLocaleString()}</td><td>$${r.cost.toFixed(2)}</td></tr>`).join('')}
    ${rows.length === 0 ? '<tr><td colspan="6" style="text-align:center;color:#666">No usage yet</td></tr>' : ''}
    <tr style="border-top:2px solid #555;font-weight:600"><td>Total</td><td></td><td>${totalRequests.toLocaleString()}</td><td>${totalInput.toLocaleString()}</td><td>${totalOutput.toLocaleString()}</td><td>$${totalCost.toFixed(2)}</td></tr>
  </table>
</div>
